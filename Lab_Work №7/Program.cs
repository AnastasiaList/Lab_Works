...
namespace Exercise__6
{
    class Geometric_figure : Type
    {
        ...
        public int Square_Figure // Исключение №1
        {
            ...
            set
            {
                if (this.Square_Figure == 0)
                { throw new Figure_Exception_2("Данные введены некорректно!", this.Square_Figure); }
                else
                { this.Square = value; }
            }
        }
    ...
    class Figure_Exception_1 : ArgumentException // Неккоректное значение
    {
        public Figure_Exception_1(string message)
            : base(message)
        { }
    }
    class Figure_Exception_2 : Exception // Неккоректное значение
    {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /* Начало лабораторной работы №7 (тест работы написанного кода) */
            ...
            try
            {
                ...
                {
                    Radius_Circle = 0,
                    Length_Circle = 2 * 3.14 * 6,
                };
            }
            catch (Exception mark)
            { Console.WriteLine($"Ошибка: {mark.Message}"); }
            finally
            { Console.WriteLine("Миссия выполнена!"); }
            try
            {
            ...
            catch (Figure_Exception_1 mark_1)
            { Console.WriteLine($"Ошибка: {mark_1.Message}"); }
            finally
            { Console.WriteLine("Миссия выполнена!"); }
            try
            {
            ...
            catch (Figure_Exception_2 mark_2)
            {
                Console.WriteLine($"Ошибка: {mark_2.Message}");
                Console.WriteLine($"Некорректное значение: {mark_2.Value}");
            }
            finally
            { Console.WriteLine("Миссия выполнена!"); }
            Debug.Assert(person_1.Black_Tea != 0, "Чёрный чай есть в каждом из нас...");
            Debug.Assert(person_1.Green_Tea == 0, "У тебя дома нет зелёного чая? А если найду?");
            /* Конец */
            Console.ReadKey();
        }
    }
